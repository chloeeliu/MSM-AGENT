...
def init_default_config() -> str:
    ...
    return yaml_dump({
        ...
        "data": {"kind": "xtc", "dir": "./ala2_test_data", "topology": "./ala2_test_data/ala2.pdb", "stride": 10, "saving_interval": 50, "load_preprocessed_dir": None},
...
class GoogleAdvisor:
    ...
    def chat_followup(self, chat_history: List[Dict[str, str]], st: SessionState) -> str:
        ...
        resp = self.client.models.generate_content(model=self.model, contents=json.dumps(input_msgs, indent=2))
...
def on_run_base(base_yaml: str, st: SessionState):
    ...
    run_dir = ""
    caught_warning_msgs: List[str] = []
    try:
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
            run_dir = run_mvp(cfg)
        caught_warning_msgs = [str(w.message) for w in caught if str(w.message).strip()]
    except AssertionError as e:
        chat.append(msg("system", f"AssertionError during base run: {e}"))
        return chat, st, "", ""
    except Exception as e:
        chat.append(msg("system", f"Base run failed: {type(e).__name__}: {e}"))
        return chat, st, "", ""

    try:
        metrics = read_json(Path(run_dir) / "metrics.json")
    except Exception as e:
        chat.append(msg("system", f"Base run finished but metrics load failed: {type(e).__name__}: {e}"))
        return chat, st, "", run_dir
    ...
    grade_warns = metrics.get("grade", {}).get("warn_reasons", []) or []
    ...
    if grade_warns:
        ...
        feedback_lines.append("Pipeline warnings: " + "; ".join(str(w) for w in grade_warns))

    if caught_warning_msgs:
        feedback_lines.append("")
        feedback_lines.append("Runtime warnings: " + "; ".join(caught_warning_msgs))

    chat.append(msg("system", "\n".join(feedback_lines)))
